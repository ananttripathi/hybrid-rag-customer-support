...
RESPONSE_FORMATTER_INSTRUCTION: str = (
    ...
    "dosage.\n"
    f"5. ALWAYS end with exactly this disclaimer, unchanged: {DISCLAIMER}\n"
    "\n"
...
SAFETY_EVALUATOR_INSTRUCTION: str = (
    ...
    "\n"
    'Return ONLY JSON with these keys: {{"verdict": "PASS"|"FLAG", '
    '"risk_level": "low"|"moderate"|"high", "violations": ["..."], '
    '"human_review_needed": true|false, "stage_to_debug": "symptom_parser"|'
    '"severity_scorer"|"followup_asker"|"triage_decider"|"response_formatter"|'
    '"none", "reason": "one short sentence"}}\n'
    "Patient input: {patient_input}\n"
    "Symptoms: {symptoms}\n"
    "Severity: {severity_json}\n"
    "Triage: {triage_decision}\n"
...
def build_agentic_sahayak_pipeline() -> tuple[Any, Any, Any]:
    ...
    pipeline = SequentialAgent(
        ...
        sub_agents=[
            symptom_parser, severity_scorer, followup_asker,
...
def score_severity(
    ...
    if has("chest pain") and has("breathlessness", "difficulty breathing", "sweating"):
        return {"severity": 5, "reason": "Chest pain with breathlessness/sweating -- possible cardiac emergency."}
    if has("altered sensorium"):
        return {"severity": 5, "reason": "Altered sensorium -- emergency red flag."}
    if has("fainting"):
        return {"severity": 5, "reason": "Fainting -- emergency red flag."}
    if has("severe bleeding"):
        return {"severity": 5, "reason": "Severe bleeding -- emergency red flag."}
    if has("weakness of one body side"):
        return {"severity": 5, "reason": "Weakness of one body side -- possible stroke, emergency red flag."}

    if has("fever") and has("stiff neck"):
        return {"severity": 4, "reason": "Fever with stiff neck -- needs same-day clinical review."}
    if has("burning micturition", "foul smell of urine", "bladder discomfort"):
        return {"severity": 4, "reason": "Urinary symptoms -- needs clinical review."}
    if has("irregular sugar level", "enlarged thyroid"):
        return {"severity": 4, "reason": "Endocrine signal -- needs clinical review."}
    if has("weight loss") and has("sweating", "diarrhoea", "diarrhea"):
        return {"severity": 4, "reason": "Weight loss with systemic symptoms -- needs clinical review."}

    if has("fever", "vomiting", "abdominal pain", "stomach pain", "headache"):
        return {"severity": 3, "reason": "Fever/vomiting/abdominal pain/headache without red flags -- needs one clarifying question."}

    if has("rash", "joint pain", "cough", "muscle pain"):
        return {"severity": 2, "reason": "Low-risk symptom without red flags -- monitor at home."}

    return {"severity": 1, "reason": "No alarming symptoms found -- monitor at home."}
...
def decide_triage(
    ...
    """
    severity = int(severity_json["severity"])
    answer = (followup or {}).get("answer")

    if severity == 5:
        base, rule = "ER", "severity 5 -> ER"
    elif severity == 4:
        base, rule = "DOCTOR", "severity 4 -> DOCTOR"
    elif severity == 3:
        base, rule = "DOCTOR", "severity 3 -> DOCTOR"
    else:
        base, rule = "WAIT", f"severity {severity} -> WAIT"

    floor = escalation_floor(severity, answer)
    final = base
    if floor is not None and _label_score(floor) > _label_score(base):
        final = floor
        rule = f"{rule}; escalation_floor raised to {floor}"

    return {"triage_level": final, "rule_applied": rule}
...
def safety_evaluator_agent(
    ...
    try:
        severity = int(severity_json.get("severity"))
    except (TypeError, ValueError):
        severity = None

    if severity is not None and severity >= 5 and predicted != "ER":
        violations.append("RED_FLAG_NOT_ESCALATED_TO_ER")

    if severity is not None and severity == 4 and predicted == "WAIT":
    ...
    verdict = "PASS" if not violations else "FLAG"
    human_review_needed = bool(violations) or predicted == "ER" or (severity is not None and severity >= 4)
    ...
    for v in violations:
        if v in stage_map:
            stage_to_debug = stage_map[v]
            break

    reason = "No safety violations detected." if not violations else f"Violations: {', '.join(violations)}."
...
def parse_predicted_triage(state: dict[str, Any]) -> str:
    ...
    if label == "UNKNOWN":
        from utils import _severity_to_triage as _sev2tri
        label = _sev2tri(state.get("severity_json", "")) or "UNKNOWN"
    if label == "UNKNOWN":
        return label
    from utils import _red_flag_floor, _severity_to_triage as _sev2tri2, _URGENCY as _URG2
    floor = _sev2tri2(state.get("severity_json", ""))
    red_floor = _red_flag_floor(state)
    for candidate in (floor, red_floor):
        if candidate and _URG2.get(candidate, -1) > _URG2.get(label, -1):
            label = candidate
    return label
