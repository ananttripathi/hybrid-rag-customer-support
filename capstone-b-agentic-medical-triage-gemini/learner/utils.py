...
_RED_FLAG_PHRASES = (
    "chest pain", "chest hurts", "difficulty breathing", "trouble breathing",
    "breathlessness", "shortness of breath", "can't breathe", "cannot breathe",
    "can not breathe", "altered sensorium", "unconscious", "unresponsive",
    "fainting", "fainted", "severe bleeding", "weakness of one body side",
    "one-sided weakness", "one sided weakness",
)


def _red_flag_floor(state):
    """ER floor when a red-flag phrase appears in symptoms or patient_input --
    mirrors the same red-flag list safety_evaluator_agent audits against."""
    text_parts = [str(state.get("patient_input", "")).lower()]
    syms = state.get("symptoms", "")
    if isinstance(syms, list):
        text_parts.append(" ".join(str(s) for s in syms).lower())
    else:
        text_parts.append(str(syms).lower())
    blob = " ".join(text_parts)
    return "ER" if any(p in blob for p in _RED_FLAG_PHRASES) else None
...
def parse_predicted_triage(state):
    ...
    if label not in ("WAIT", "DOCTOR", "ER"):
        label = _severity_to_triage(state.get("severity_json", "")) or "UNKNOWN"
    if label == "UNKNOWN":
        return label
    # Deterministic raise-only safety floor: never let a parsed/recovered label
    # sit below what severity_json or an explicit red-flag phrase implies.
    # Mirrors the escalation_floor() / decide_triage() pattern already used
    # for the policy engine -- extended here to the ADK agent's final label.
    floor = _severity_to_triage(state.get("severity_json", ""))
    red_floor = _red_flag_floor(state)
    for candidate in (floor, red_floor):
        if candidate and _URGENCY.get(candidate, -1) > _URGENCY.get(label, -1):
            label = candidate
    return label
