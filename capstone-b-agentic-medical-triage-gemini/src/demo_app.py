...
# falls back to local Ollama hermes3:8b otherwise. Variable name kept as OLLAMA_MODEL
# to avoid touching every LlmAgent(..., model=OLLAMA_MODEL) call site below.
if _GEMINI_KEY and _GEMINI_KEY != "dummy":
    OLLAMA_MODEL = "gemini-3.5-flash"
else:
...
_IS_GEMINI = OLLAMA_MODEL == "gemini-3.5-flash"
_MODEL_LABEL = "gemini-3.5-flash" if _IS_GEMINI else "hermes3:8b"
_BACKEND_LABEL = "Gemini (Google AI Studio)" if _IS_GEMINI else "Ollama local"

def _label_active_model(html: str) -> str:
    return (
        html.replace("hermes3:8b &middot; Ollama local", f"{_MODEL_LABEL} &middot; {_BACKEND_LABEL}")
        .replace('<div class="num gray" style="font-size:14px;margin-top:3px">hermes3:8b</div><div class="sub">Ollama local</div>',
                  f'<div class="num gray" style="font-size:14px;margin-top:3px">{_MODEL_LABEL}</div><div class="sub">{_BACKEND_LABEL}</div>')
        .replace("hermes3:8b on Ollama.", f"{_MODEL_LABEL} via {_BACKEND_LABEL}." if _IS_GEMINI else "hermes3:8b on Ollama.")
        .replace("hermes3:8b &middot; two-phase pipeline", f"{_MODEL_LABEL} &middot; two-phase pipeline")
    )

DASHBOARD_HTML = _label_active_model(DASHBOARD_HTML)
HTML = _label_active_model(HTML)
...
if __name__ == "__main__":
    ...
    print(f"  Model: gemini-3.5-flash (Google AI Studio)" if OLLAMA_MODEL == "gemini-3.5-flash" else f"  Model: {MODEL_ID} via Ollama")
